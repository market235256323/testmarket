...
export default function ProductForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // შევამოწმოთ ატვირთულია თუ არა მინიმუმ 1 ფოტო
    ...
    <div className="max-w-4xl mx-auto bg-white/90 rounded-lg shadow-lg p-6 border border-blue-200 text-blue-900">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-center pt-4">
          <button
            type="submit"
            disabled={isSubmitting || isUploading || (imageFiles.length === 0 && formData.imageUrls.length === 0)}
            className={`px-6 py-2 ${
              imageFiles.length === 0 && formData.imageUrls.length === 0 
                ? "bg-gray-400 cursor-not-allowed" 
                : "bg-blue-600 hover:bg-blue-700"
            } text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 disabled:opacity-50 transition-colors flex items-center justify-center min-w-[180px]`}
          >
